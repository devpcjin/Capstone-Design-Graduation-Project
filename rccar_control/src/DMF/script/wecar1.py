...
class LineCar:
    ...
    def recognition(self):
        ...
        low_val = (39, 0, 220) #39, 0, 184
        high_val = (221, 255, 255)

        #low_val = (100, 220, 80)
        #high_val = (120, 255, 255)

        mask = cv2.inRange(hsv, low_val, high_val)

        #bgr_low = (150, 150, 150)
        #bgr_high = (255, 255, 255)
        #mask = cv.inRange(temp, bgr_low, bgr_high)
        #cv2.imshow("ROI", mask)
    ...
    def judgement(self, _lines):
        if _lines is not None:
            ...
            line_arr = line_arr[np.abs(slope_degree) < L_temp_val]
            slope_degree = slope_degree[np.abs(slope_degree) < L_temp_val]


            line_arr = line_arr[np.abs(slope_degree) > R_temp_val]
            slope_degree = slope_degree[np.abs(slope_degree) > R_temp_val]
            ...
            if len(R_lines) > 0 and len(L_lines) == 0:
                ...
                _lines = np.concatenate([L_lines, R_lines], axis = 0)
            else:
                _lines = []
...
def run_line_detect(_steer):
    run_car = LineCar()
    run_car.set_cam(1)
    run_car.cam_setting(0)
    run_car.detect_line(_steer)
...
if __name__ == '__main__':
    ...
    gflag = Manager().Value(ctypes.c_int8, -3)
